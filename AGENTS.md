...
- `main` is **protected** - direct pushes are rejected, so `git push origin main` never
  works. Branch off an up-to-date `main` BEFORE staging, `git add -- <explicit paths>`
  (never `git add -A`), commit, `git push -u origin <branch>`, `gh pr create --base main
  --fill`, wait for CI with `gh pr checks <branch> --watch`, then
  `gh pr merge <branch> --squash --delete-branch`. Merging deploys to GitHub Pages.
  Never merge with a required check failing or pending. Full flow: @CLAUDE.md.
